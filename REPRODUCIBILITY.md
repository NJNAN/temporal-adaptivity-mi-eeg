...
- `MI-Mamba-style` implementation card: the classifier uses an EEGNet-like spatial frontend followed by two selective-SSM blocks with `d_model=64` and `d_state=16`. Each block applies layer normalization, a `Linear(d_model, 2*d_model)` projection split into content and sigmoid gate streams, a depthwise temporal convolution with kernel size 5, input-dependent `B(x_t)` and `C(x_t)` projections of shape `(d_model, d_state)`, a negative stable state matrix `A=-softplus(A_log)` initialized from zeros, and a fixed-step exponential scan `state_t = exp(A) * state_{t-1} + B(x_t) * x_t`. The block output is `(C(x_t) * state_t).sum(-1)`, gated, projected, dropped out with `p=0.2`, and added residually. The final readout uses layer normalization followed by temporal mean-plus-max pooling and a linear classifier. This controlled surrogate preserves input-dependent gating and selective state scanning while avoiding the `mamba-ssm` CUDA extension.
...
- The existing Euclidean-alignment LOSO output is a Riemann-TSLR diagnostic only. A fair aligned cross-subject comparison would need to apply the same unsupervised alignment/adaptation setting to all neural baselines and report aligned and unaligned rankings separately.
